...
/*
    ...
    */
    fn.remove_widget = function(el, callback) {
        ...
        if ($nexts.length > 0) {
            ...
            $nexts = this.widgets_right($el);            

            $el.fadeOut($.proxy(function() {
                $el.remove();

                $nexts.each($.proxy(function(i, widget) {
                    this.move_widget_left( $(widget), wgd.size_x );
                }, this));
    ...
    */
    ...
    /**
    * Move up the specified widget and all to the right of it.
    *
    * @method move_widget_left
    * @param {HTMLElement} $widget The widget you want to move.
    * @param {Number} [y_units] The number of cells that the widget has to move.
    * @return {Class} Returns the instance of the Gridster Class.
    */
    fn.move_widget_left = function($widget, x_units) {
        var el_grid_data = $widget.coords().grid;
        var actual_col = el_grid_data.col;
        var moved = [];
        var can_go_up = true;
        x_units || (x_units = 1);

        if (!this.can_move_to($widget, el_grid_data.col - 1, el_grid_data.row)) { return false; } //break;

        this.for_each_column_occupied(el_grid_data, function(row) {
            // can_go_up
            if ($.inArray($widget, moved) === -1) {
                var widget_grid_data = $widget.coords().grid;
                var next_col = actual_col - x_units;
                next_col = this.can_go_up_to_col(
                    widget_grid_data, next_col, row);

                if (!next_col) {
                    return true;
                }

                var $next_widgets = this.widgets_right($widget);
                ...
                $next_widgets.each($.proxy(function(i, widget) {
                    this.move_widget_left($(widget), x_units);
                }, this));
            }
        });
    ...
    */
    ...
    /**
    * Check if the widget can move to the specified col, else returns the
    * most left col possible.
    *
    * @method can_go_up_to_col
    * @param {Number} widget_grid_data The current grid coords object of the
    *  widget.
    * @param {Number} col The target column.
    * @param {Number} row The target row.
    * @return {Boolean|Number} Returns the col number if the widget can move
    *  to the target position, else returns false.
    */
    fn.can_go_up_to_col = function(widget_grid_data, col, row) {
        var ga = this.gridmap;
        var result = true;
        var urc = []; // upper_rows_in_columns
        ...
        /* generate an array with columns as index and array with
         * upper rows empty in the column */
        this.for_each_row_occupied(widget_grid_data, function(trow) {
            var grid_row = ga[trow];
            urc[trow] = [];

            c = actual_col;
            while (c--) {
                if (this.is_empty(c, trow) &&
                    !this.is_placeholder_in(c, trow)
                ) {
                    urc[trow].push(c);
                }else{
                    break;
        ...
        /* get common rows starting from the most left position in all the rows
         * that widget occupies */
        c = col;
        for (c = 1; c < actual_col; c++) {
            var common = true;

            for (var uc = 0, ucl = urc.length; uc < ucl; uc++) {
                if (urc[uc] && $.inArray(c, urc[uc]) === -1) {
                    common = false;
                }
            }
    ...
    */
    ...
    /**
    * Get widgets at the right of a widget.
    *
    * @method widgets_right
    * @param {HTMLElement} $el The jQuery wrapped HTMLElement.
    * @return {HTMLElements} A jQuery collection of HTMLElements.
    */
    fn.widgets_right = function($el) {
        var el_grid_data = $.isPlainObject($el) ? $el : $el.coords().grid;
        var self = this;
        var ga = this.gridmap;
        var next_row = el_grid_data.row + el_grid_data.size_y - 1;
        ...
        this.for_each_column_occupied(el_grid_data, function(col) {
            self.for_each_widget_right(col, next_row, function(tcol, trow) {
                if (!self.is_player(this) && $.inArray(this, $nexts) === -1) {
                    $nexts = $nexts.add(this);
                    return true; // break
                }
        ...
        return this.sort_by_col_asc($nexts);
    };
    ...
    */
    ...
    fn._traversing_widgets = function(type, direction, col, row, callback) {
        ...
        var methods = {
            ...
            'for_each/right': function() {
                for (tcol = col + 1, max = ga.length; tcol < max; tcol++) {
                    if (this.is_widget(tcol, row) &&
                        $.inArray(ga[tcol][row], matched) === -1
                    ) {
                        cr = callback.call(ga[tcol][row], tcol, row);
                        matched.push(ga[tcol][row]);
                        if (cr) { break; }
                    }
                }
            }
    ...
    */
    ...
    /**
    * Iterate over each widget to the right of the column and row specified.
    *
    * @method for_each_widget_right
    * @param {Number} col The column to start iterating.
    * @param {Number} row The row to start iterating.
    * @param {Function} callback The function to execute on each widget
    *  iteration. The value of `this` inside the function is the jQuery wrapped
    ...
    * @return {Class} Returns the instance of the Gridster Class.
    */
    fn.for_each_widget_right = function(col, row, callback) {
        this._traversing_widgets('for_each', 'right', col, row, callback);
        return this;
    };
