...
module.exports = Class.extend({
	...
	import: function(filter) {
		console.info('Importing %s%s', this.itemsLabel(), filterToString(filter));
		this.processedIds = {};
		var start = Date.now();
		var self = this;
		...
		.then(function(items) {
			...
			console.info('Processing %d %s%s...', items.length, self.itemsLabel(), filterToString(filter))
			return self.processAll(items, filter);
		})
		.then(function() {
			// Notify by log that the process finished
			var end = Date.now();
		    console.info('FINISHED IMPORTING the %s%s in %d seconds', self.itemsLabel(), filterToString(filter), (end-start)/1000);
		})
		.fail(function(err) {
			// Error occurred while procesing, log and throw it up
			console.error('Problem while importing the ' + self.itemsLabel(), err);
			//throw err;
			return true;
		})
		.fin(function() {
	...
	loadItemList: function(filter) {
		console.trace('Retreiving ' + this.itemLabel() + ' list');
		...
		.fail(function(err) {
			throw new Error('Problem while loading list of ' + self.itemsLabel() + ': ' + errorToString(err));
		});
	},
	...
	removeAll: function(filter) {
		if(filter) {
			console.log('Removing ALL %s using filter %s', this.itemsLabel(), JSON.stringify(filter));
		} else {
			console.log('Removing ALL %s', this.itemsLabel());
			filter = {};
		}
		...
		return Promise.asPromise(this.config.Model, this.config.Model.remove, filter)
			.fail(function(err) {
				throw new Error('Problem while removing all ' + self.itemsLabel() + ': ' + errorToString(err));
			});
	},
	...
	processItemsSequentially: function(filter, items, i) {
		...
		// Due to a XBMC bug, the parent id sometimes is incorrect (eg. wrong tvshowid for episodes), but it stills return them when filtering
		// by the right id (yes, weird!). So we ensure the right id is set on the item.
		if(filter) _.extend(it, filter);

		return this.processItem(it)
			.then(function(item) {
				// Notify the item has been processed and update the % done
				console.log("%d% of the %s%s IMPORTED: %s", Math.round((i+1)/items.length*100), self.itemsLabel(), filterToString(filter), self.itemToString(item));
				return item;
			})
			.then(function(item) {
	...
	processItem: function(item) {
		...
		if(this.processedIds[itemId]) {
			console.trace("%s already processed, skipping...", this.itemToString(item));
			return Promise.resolve(item);
		}
		this.processedIds[itemId] = true;
			
		console.trace('Processing %s: %s', this.itemLabel(), this.itemToString(item));
		var self = this;
	...
	persistItem: function(item) {
		console.trace('Persisting to DB %s', this.itemToString(item));
		// Save item in cache
	    ...
	    return promise.fail(function(err) {
			throw new Error('Problem while persisting ' + self.itemToString(item) + ': ' + errorToString(err));
		});
	},
	...
	importItem: function(i) {
		console.info('Importing %s => %s', this.itemLabel(), this.itemToString(i));
		var self = this;
		...
		.fail(function(err) {
			console.error("There was a problem while importing " + this.itemLabel() + ' => ' + this.itemToString(i), err);
		});
	},
	...
	removeItemById: function(itemId) {
		var params = {};
		params[this.config.idField] = itemId;
	...
	removeItem: function(item) {
		...
		var itemId = this.id(item);
		console.trace('Removing from DB %s', this.idToString(itemId));
		...
		params[this.config.idField] = itemId;
		// Call Mongoose
		return Promise.asPromise(this.config.Model, this.config.Model.remove, params)
			.fail(function(err) {
				throw new Error('Problem while removing ' + self.idToString(itemId) + ' details: ' + errorToString(err));
			});
	},
	...
	processChildren: function(item) {
		if(this.config.subImporter) {
			...
			console.trace('Importing %s for %s', this.config.subImporter.itemsLabel(), this.itemToString(item));
	...
	loadItemDetails: function(item) {
		...
		return promise.then(function(res) {
			...
			throw new Error('Problem while loading ' + self.idToString(id) + ': ' + errorToString(err));
		});
	},
