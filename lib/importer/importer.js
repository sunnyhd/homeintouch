...
module.exports = Class.extend({
	...
	processAll: function(items, filter) {
		if(items.length === 0) return Promise.asPromise([]);
	...
	processItemsSequentially: function(filter, items, i) {
		...
		return this.processItem(items[i])
			...
			.then(function(item) {
				if(i >= items.length - 1) return Promise.asPromise(true);
	...
	processItem: function(i) {
		...
		if(this.processedIds[itemId]) {
			console.log(this.itemToString(i) + " already processed, skipping...");
			return Promise.asPromise(i);
		}
	...
	loadItemList: function(filter) {
		...
		return promise.then(function(res) {
			// Only return the attribute defined in the configuration from the response
			var items = res[self.config.list.responseField];
			if(!items) items = [];
			return items;
		}).fail(function(err) {
