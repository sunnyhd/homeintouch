...
mediaManager.onMediaCenterLibraryUpdate = function(data) {
    ...
    if (op) {
        ...
        if(op === 'add') {
        	...
        	console.info('Removing %s with id %s', item.type, item.id);
        	promise = importer.removeItemById(item.id);
        }

        return promise.then(function() {
        	console.info('%s with id %s updated successfully', item.type, item.id);
    	...
    	console.trace('The XBMC notification is not handled by the server, forwarding it to the client');
    	// If the notification is about something else, just send it with no processing
