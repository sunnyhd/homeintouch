 ...
 */
mediaManager.import = function(importMedia) {
	...
	var mediaToImport = getImporter(importMedia);

	if (mediaToImport) {
		Promise.all(mediaToImport.map(function(importer) { return importer.import(); }))
		.then(function(){
			var end = Date.now();
		...
		console.log('Invalid media name, it should be: music, movie, tvshow');
	}
}
...
mediaManager.importMusic = function() {
	...
	albumImporter.import().done();
	songImporter.import().done()
}
...
function getImporter(type) {
	switch(type.toLowerCase()) {
		...
		case 'tvshow': return [tvShowImporter, episodeImporter];
