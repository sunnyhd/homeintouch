...
eib.connect = function() {
  ...
  child.stdout.on("data", function(data) {
    ...
    var id = data[0].split(" ").pop()
      ...
      , oldvalue = parseInt(bytes.join(""), 16)
      , newvalue = parseInt(bytes[0], 16) * 16 + parseInt(bytes[1], 16)

    if (bytes.length == 2) value /= 100

    eib.emit("rawAddress", id, data[1])
    eib.emit("address", id, oldvalue)
    eib.emit("newAddress", id, newvalue)
  })
